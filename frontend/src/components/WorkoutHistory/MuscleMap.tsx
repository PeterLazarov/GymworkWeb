...
function getMappedClasses(muscles: string[]) {
  return Array.from(
    new Set(muscles.flatMap((m) => mappings[m as keyof typeof mappings] ?? []))
  );
}
...
const MuscleMapComponent = ({
  ...
  const styleSheet = useMemo(() => {
    const activeClassesStr = activeClasses
      .map((cls) => `.${cls} { fill: ${activeColor}; }`)
      .join("\n");
    return `
      #Base { fill: ${baseColor}; }
      #Front { opacity: ${back ? 0 : 1}; }
      #Back { opacity: ${back ? 1 : 0}; }
  ...
  return (
    ...
    >
      ...
      />
      <g id="Back" style={{ opacity: back ? 1 : 0 }}>
        <BackMuscleSVGMap />
      </g>
      <g id="Front" style={{ opacity: back ? 0 : 1 }}>
        <FrontMuscleSVGMap />
      </g>
    </svg>
  );
